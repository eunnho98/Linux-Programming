...
// Dining, Deadlock Free Version
// hold and wait을 하지 않도록 함
...
// 내가 배고프고 왼쪽, 오른쪽 철학자가 먹지않는 상태이면 먹을 수 있음
void
Test(int id)
{
	if (State[id] == HUNGRY && State[LEFT(id)] != EATING 
		...
		if (sem_post(&Philosopher[id]) < 0)  { // test 통과하면 post해서 먹을 수 있도록
			perror("sem_post");
			pthread_exit(NULL);
		}
	}
...
// 내려놓을 때 양 옆 철학자들을 테스트해서 먹을 수 있도록
	Test(LEFT(id));
	Test(RIGHT(id));
