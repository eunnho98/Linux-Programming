...
DiningPhilosopher(int *pId)
{
	...
	for (i = 0 ; i < NLOOPS ; i++)  {
		...
		if (sem_wait(&ChopStick[(id+1) % NUM_MEN]) < 0)  { // 오른쪽 젓가락 들기
			perror("sem_wait");
			pthread_exit(NULL);
		}
		...
		if (sem_post(&ChopStick[id]) < 0)  { // 왼쪽 젓가락 놓기
			perror("sem_post");
			pthread_exit(NULL);
		}
		if (sem_post(&ChopStick[(id+1) % NUM_MEN]) < 0)  { // 오른쪽 젓가락 놓기
			perror("sem_post");
			pthread_exit(NULL);
		}
	}
